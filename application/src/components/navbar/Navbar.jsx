...
function Navbar (props) {
    ...
    const [themeSvg, setThemeSvg] = useState("dark");

    useEffect(() => {

        const savedTheme = localStorage.getItem("theme") || "dark";
        setThemeSvg(savedTheme);
    ...
    const toggleTheme = () => {
        const newTheme = themeSvg === "light" ? "dark" : "light";
        setThemeSvg(newTheme);
    ...
    return (
        <nav className="navbar-container d-flex justify-content-between align-items-center">
            ...
            <div className="navbar-links d-flex justify-content-center align-items-center">
                <NavbarLink 
                    condition={props.themeCondition} 
                    path="#theme"
                    svg={
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" 
                            className={`bi bi-${themeSvg === "light" ? "sun" : "moon-stars"}`} 
                            viewBox="0 0 16 16">
                            {themeSvg === "light" ? (
                                <>
                ...
                {!props.sessionActive ?
                    <NavbarLink
                        condition={props.register}
                        path="/register"
                        svg={<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-pencil-square" viewBox="0 0 16 16">
                ...
                <NavbarLink 
                    condition={props.login}
                    path="/login"
                    svg={props.sessionActive ? 
                    ...
                    text={props.sessionActive ? "Account" : "Login"}
                />

                <NavbarLink 
                    condition={props.logout}
                    path="#logout"
                    svg={<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" className="bi bi-box-arrow-in-left" viewBox="0 0 16 16">
